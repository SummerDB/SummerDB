#ifndef SUMMERDB_COMMON_TYPES_HASH_HPP
#define SUMMERDB_COMMON_TYPES_HASH_HPP
...
template <class T>
int32_t Hash(T value) {
  return murmurhash32(value);
}

template <>
int32_t Hash(uint64_t val);
template <>
int32_t Hash(int64_t val);
template <>
int32_t Hash(double val);
template <>
int32_t Hash(const char* val);
template <>
int32_t Hash(char* val);

}  // namespace SummerDB

#endif  // SUMMERDB_COMMON_TYPES_HASH_HPP