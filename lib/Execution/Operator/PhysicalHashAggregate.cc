...
PhysicalHashAggregate::PhysicalHashAggregate(
    ...
    : PhysicalAggregate(std::move(expressions),
...
PhysicalHashAggregate::PhysicalHashAggregate(
    ...
    : PhysicalAggregate(std::move(expressions), std::move(groups),
                        PhysicalOperatorType::HASH_GROUP_BY) {
  Initialize();
}
...
std::unique_ptr<PhysicalOperatorState> PhysicalHashAggregate::GetOperatorState(
  ...
  if (groups.size() > 0) {
    std::vector<TypeId> group_types, payload_types;
    ...
    state->ht = std::make_unique<SuperLargeHashTable>(
