#include "SummerDB/Execution/Operator/PhysicalLimit.hpp"

namespace SummerDB {

std::vector<TypeId> PhysicalLimit::GetTypes() {
...
void PhysicalLimit::_GetChunk(ClientContext& context, DataChunk& chunk,
  ...
  if (state->current_offset < offset) {
    ...
    if (state->current_offset + state->child_chunk.count >= offset) {
      ...
      chunk.count = std::min(limit, state->child_chunk.count - start_position);
      for (size_t i = 0; i < chunk.column_count; i++) {
        chunk.data[i].Reference(state->child_chunk.data[i]);
        chunk.data[i].data = chunk.data[i].data +
...
std::unique_ptr<PhysicalOperatorState> PhysicalLimit::GetOperatorState(
    ExpressionExecutor* parent_executor) {
  return std::make_unique<PhysicalLimitOperatorState>(children[0].get(), 0,
                                                      parent_executor);
}
