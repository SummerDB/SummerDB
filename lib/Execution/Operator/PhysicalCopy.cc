#include "SummerDB/Execution/Operator/PhysicalCopy.hpp"
...
std::vector<std::string> split(const std::string& str, char delimiter, char quote) {
...
void PhysicalCopy::_GetChunk(ClientContext& context, DataChunk& chunk,
  ...
  if (table) {
    ...
    while (getline(from_csv, value)) {
      ...
      std::vector<std::string> csv_line = split(value, delimiter, quote);

      if (select_list.size()) {
        for (size_t i = 0; i < table->columns.size(); ++i) {
          if (!(std::find(select_list_oid.begin(), select_list_oid.end(), i) !=
                select_list_oid.end())) {
...
std::unique_ptr<PhysicalOperatorState> PhysicalCopy::GetOperatorState(
