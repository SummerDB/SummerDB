...
#include "SummerDB/Parser/Expression/ColumnRefExpression.hpp"

namespace SummerDB {

std::string BindContext::GetMatchingTable(const std::string& column_name) {
  std::string result;
  if (dummy_table) {
    // bind to dummy table
    return "";
  ...
  if (result.empty()) {
    ...
    throw BinderException("Referenced column \"%s\" not found in FROM clause!",
                          column_name.c_str());
  }
  return result;
}
...
ColumnDefinition* BindContext::BindColumn(ColumnRefExpression& expr,
  ...
  if (!HasAlias(expr.table_name)) {
    if (parent) {
      ...
      max_depth = std::max(expr.depth, max_depth);
      return result;
    }
  ...
  if (table_entry != regular_table_alias_map.end()) {
    ...
    if (!table.table->ColumnExists(expr.column_name)) {
      throw BinderException("Table \"%s\" does not have a column named \"%s\"",
  ...
  for (auto table_name : regular_table_alias_list) {
    ...
    for (auto& column : table.table->columns) {
      new_select_list.push_back(
          std::make_unique<ColumnRefExpression>(column.name, table_name));
    }
  }
  for (auto& kv : subquery_alias_map) {
    auto subquery = kv.second;
...
void BindContext::AddDummyTable(std::vector<ColumnDefinition>& columns) {
  ...
  dummy_table = std::make_unique<DummyTableBinding>(columns);
}
...
void BindContext::AddExpression(const std::string& alias,
                                Expression* expression, size_t i) {
  expression_alias_map[alias] = std::make_pair(i, expression);
}

bool BindContext::HasAlias(const std::string& alias) {
  return regular_table_alias_map.find(alias) != regular_table_alias_map.end() ||
