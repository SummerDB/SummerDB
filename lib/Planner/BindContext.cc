...
std::shared_ptr<ColumnCatalogEntry> BindContext::BindColumn(
    ColumnRefExpression& expr) {
  ...
  for (auto& kv : regular_table_alias_map) {
    ...
    for (auto& column : table->columns) {
      ...
      new_select_list.push_back(
          std::make_unique<ColumnRefExpression>(column_name, table_name));
    }
  }
  for (auto& kv : subquery_alias_map) {
    auto subquery = kv.second;
...
void BindContext::AddExpression(const std::string& alias,
  ...
  expression_alias_map[alias] =
      std::pair<size_t, AbstractExpression*>(i, expression);
}
