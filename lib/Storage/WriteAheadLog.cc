...
void WriteAheadLog::Replay(std::string& path) {
  ...
  std::vector<WALEntryData> stored_entries;
  WALEntry entry;
  while (ReadEntry(entry, wal_file)) {
    ...
    if (entry.type == WALEntry::WAL_FLUSH) {
      ...
      WALEntryData data;
      data.entry = entry;
      data.data = std::unique_ptr<uint8_t[]>(new uint8_t[entry.size]);
      // read the data
      ...
      stored_entries.push_back(std::move(data));
    }
  }
  fclose(wal_file);
}
...
//===--------------------------------------------------------------------===//
template <class T>
void WriteAheadLog::Write(T val) {
...
bool ReplayCreateTable(Transaction& transaction, Catalog& catalog,
  ...
  info.schema = source.Read<std::string>(failed);
  info.table = source.Read<std::string>(failed);
...
bool ReplayDropTable(Transaction& transaction, Catalog& catalog,
                     Deserializer& source) {
  bool failed = false;
  auto schema_name = source.Read<std::string>(failed);
  auto table_name = source.Read<std::string>(failed);
  if (failed) {
    return false;
  }
...
bool ReplayQuery(ClientContext& context, Deserializer& source) {
  // read the query
  bool failed = false;
  auto query = source.Read<std::string>(failed);
  if (failed) {
    return false;
  }

  auto result = SummerDBConnection::GetQueryResult(context, query);
