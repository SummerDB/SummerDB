...
void QueryProfiler::StartOperator(PhysicalOperator* phys_op) {
  ...
  if (tree_map.count(phys_op) == 0) {
    ...
    node->children.push_back(std::move(new_tree));
  }
...
std::unique_ptr<QueryProfiler::TreeNode> QueryProfiler::CreateTree(
    PhysicalOperator* root, size_t depth) {
  ...
  for (size_t i = 0; i < TREE_RENDER_HEIGHT; i++) {
    if (render[start_depth + i].size() > start_position) {
      ...
      render[start_depth + i] +=
          std::string(start_position - render[start_depth + i].size(), ' ');
    }
  }
