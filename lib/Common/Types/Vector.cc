...
void Vector::Initialize(TypeId new_type, bool zero_data) {
  ...
  string_heap.Destroy();
  owns_data = true;
  owned_data = std::unique_ptr<char[]>(
...
void Vector::SetStringValue(size_t index, const char* value) {
  if (type != TypeId::VARCHAR) {
    throw InvalidTypeException(type,
                               "Can only set string value of VARCHAR vectors!");
  }
...
Value Vector::GetValue(size_t index) const {
  ...
  switch (type) {
    ...
    case TypeId::VARCHAR: {
      ...
      return !str ? Value(TypeId::VARCHAR) : Value(std::string(str));
    }
    default:
...
void Vector::Copy(Vector& other, size_t offset) {
  ...
  if (!TypeIsConstantSize(type)) {
    ...
    for (size_t i = 0; i < other.count; i++) {
      const char* str =
          sel_vector ? source[sel_vector[i + offset]] : source[i + offset];
...
std::string Vector::ToString() const {
  std::string retval =
      TypeIdToString(type) + ": " + std::to_string(count) + " = [ ";
  for (size_t i = 0; i < count; i++) {
