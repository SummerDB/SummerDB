...
void DataChunk::Initialize(std::vector<TypeId>& types, bool zero_data) {
  ...
  if (size > 0) {
    owned_data = std::unique_ptr<char[]>(new char[size]);
    if (zero_data) {
  ...
  data = std::unique_ptr<Vector[]>(new Vector[types.size()]);
...
void DataChunk::Move(DataChunk& other) {
  ...
  other.owned_data = std::move(owned_data);
  if (sel_vector) {
...
void DataChunk::SetSelectionVector(Vector& matches) {
  if (matches.type != TypeId::BOOLEAN) {
    throw InvalidTypeException(
        matches.type, "Can only set selection vector using a boolean vector!");
  }
...
std::vector<TypeId> DataChunk::GetTypes() {
...
std::string DataChunk::ToString() const {
  std::string retval =
      "Chunk - [" + std::to_string(column_count) + " Columns]\n";
  for (size_t i = 0; i < column_count; i++) {
  ...
  for (size_t i = 0; i < column_count; i++) {
    ...
    if (TypeIsConstantSize(type)) {
      ...
      for (size_t j = 0; j < rows; j++) {
        // read the strings
        auto str = source.Read<std::string>(failed);
        if (failed) {
          return false;
        }
