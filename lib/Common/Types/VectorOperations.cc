  ...
  if (left.count == 1 && !left.sel_vector) {
    ...
    if (left.sel_vector) {
      ...
      for (size_t i = 0; i < left.count; i++) {
        ...
        result_data[left.sel_vector[i]] =
            OP::Operation(ldata[left.sel_vector[i]], rdata[left.sel_vector[i]]);
      }
    } else {
      assert(!right.sel_vector);
      for (size_t i = 0; i < left.count; i++) {
...
void _is_null_loop(Vector& left, Vector& result) {
  ...
  if (left.sel_vector) {
    for (size_t i = 0; i < left.count; i++) {
      res[left.sel_vector[i]] = INVERSE ? !left.nullmask[left.sel_vector[i]]
...
void VectorOperations::Equals(Vector& left, Vector& right, Vector& result) {
  if (left.type == TypeId::VARCHAR) {
    ...
    _templated_binary_loop<char*, int8_t, Operators::EqualsVarchar>(left, right,
                                                                    result);
  } else {
...
void VectorOperations::GreaterThanEquals(Vector& left, Vector& right,
  ...
  if (left.type == TypeId::VARCHAR) {
    ...
    _templated_binary_loop<char*, int8_t, Operators::GreaterThanEqualsVarchar>(
        left, right, result);
  } else {
  ...
  if (left.count == right.count) {
    if (left.sel_vector) {
      ...
      for (size_t i = 0; i < left.count; i++) {
        ...
        result.nullmask[index] =
            NULLOP::Operation(ldata[index], rdata[index], left.nullmask[index],
    ...
    if (right.sel_vector) {
      ...
      for (size_t i = 0; i < right.count; i++) {
        result_data[i] = OP::Operation(constant, rdata[i]);
        result.nullmask[i] =
            NULLOP::Operation(constant, rdata[i], left_null, right.nullmask[i]);
      }
    }
    result.sel_vector = right.sel_vector;
    result.count = right.count;
